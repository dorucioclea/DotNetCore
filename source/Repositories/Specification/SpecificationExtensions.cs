...
namespace DotNetCore.Repositories
{
    public static class SpecificationExtensions
    {
        ...
        {
            return specification.OrderBy == default || specification.OrderByDescending != default ? queryable : queryable.OrderBy(specification.OrderBy);
        }

        private static IQueryable<T> OrderByDescending<T>(this IQueryable<T> queryable, ISpecification<T> specification) where T : class
        {
            return specification.OrderByDescending == default || specification.OrderBy != default ? queryable : queryable.OrderByDescending(specification.OrderByDescending);
        }
        ...
        {
            return specification.Where == default ? queryable : queryable.Where(specification.Where);
        }
    }
}
